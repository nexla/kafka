...
- `cp` `kafka-client-2.8.1.jar` to `/tmp/kafka-client/kafka-clients-2.8.1-hack.jar`
- `cp -R backend-connectors/common-connector/target/classes/org /tmp/kafka-client/`
- `cd /tmp/kafka-client && jar uf kafka-clients-2.8.1-hack.jar  org/apache/kafka/clients/consumer/internals/`
- Generate `kafka-clients-2.8.1-hack.jar.sha1`,	`kafka-clients-2.8.1-hack.pom` & `kafka-clients-2.8.1-hack.pom.sha1`
    - `kafka-clients-2.8.1-hack.pom` can be generated by just copying `kafka-clients-2.8.1.pom` and changing the version to `3.9.0-hack`
...
git checkout -b 2.8.1 2.8.1
# Push the changes 
git push -u origin refs/heads/2.8.1
# Create a branch from the tag or branch 2.8.1 and make the changes there
```
