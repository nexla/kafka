...
Along with the custom changes in the branch. It also has changes in `ConsumerCoordinator.java` placed under `backend-connectors` repo. Relevant PRs
1. https://github.com/nexla/backend-connectors/pull/1038/

- Build `kafka-client` by following the steps outlined under next section.
- Build `backend-connectors`
- `cp` `kafka-client-3.9.0.jar` to `/tmp/kafka-client/kafka-clients-3.9.0-hack.jar`
- `cp -R backend-connectors/common-connector/target/classes/org /tmp/kafka-client/`
- `cd /tmp/kafka-client && jar uf kafka-clients-3.9.0-hack.jar  org/apache/kafka/clients/consumer/internals/`
- Generate `kafka-clients-3.9.0-hack.jar.sha1`,	`kafka-clients-3.9.0-hack.pom` & `kafka-clients-3.9.0-hack.pom.sha1`
    - `kafka-clients-3.9.0-hack.pom` can be generated by just copying `kafka-clients-3.9.0.pom` and changing the version to `3.9.0-hack`
...
Steps are outlined [here](https://nexla1.atlassian.net/browse/NEX-18101).

```bash

# Kafka client release tag for each release. 
# Github does not allow to raise PR against tags, so create a branch from the tag.
git checkout -b 3.9.0 3.9.0
# Push the changes 
git push -u origin refs/heads/3.9.0
# Create a branch from the tag or branch 3.9.0 and make the changes there
